"""Professional optional Textual workspace for FenrirAgent."""
...
from fenrir_agent.command_registry import match_commands
...
class TextualPermissionBroker:
    ...
    def __init__(self, app: "FenrirAgentTui") -> None:
        self.app = app
        self._closed = False
...
class FenrirAgentTui(App[None]):
    ...
    def compose(self) -> ComposeResult:
        yield Static("Fenrir Agent", id="identity")
    ...
    def _select_model(self, key: str | None) -> None:
        ...
        if key.startswith("api::"):
            ...
            if not profile:
                return
            from fenrir_agent.api_providers import PROVIDERS
    ...
    def _create_model_profile(self, values: dict[str, str] | None) -> None:
        ...
        from fenrir_agent.model_registry import ModelRegistryError
    ...
    def _discover_api_models(self, provider: str, api_key: str) -> None:
        from fenrir_agent.api_providers import ApiProviderError, OpenAICompatibleClient
    ...
    def _refresh_state(self) -> None:
        ...
        self.query_one("#identity", Static).update(
            f"Fenrir Agent · {snapshot.profile.display_name} · {cwd}"
        )
...
__all__ = ["ChoiceScreen", "FenrirAgentTui", "PermissionScreen", "TextualPermissionBroker"]
