...
class ModelCapabilityProfile:
    """Capabilities FenrirAgent needs for safe context and feature decisions."""
...
class ModelProfileRegistry:
    ...
    def __init__(self, workspace: Path, config_path: Optional[Path] = None):
        ...
        self.config_path = config_path or self.workspace / ".fenrir" / "config.toml"
    ...
    def _load_overrides(self) -> Dict[str, Dict[str, Any]]:
        ...
        if not isinstance(models, dict):
            self._warn("[models] in .fenrir/config.toml must be a table")
            return {}
        return {
