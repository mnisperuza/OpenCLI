"""Bounded, citation-preserving web search for FenrirAgent.
...
_MAX_ARXIV_RESPONSE_BYTES = 2_000_000
...
class WebRetriever:
    ...
    def _search_arxiv(self, query: str, limit: int) -> list[Dict[str, Any]]:
        ...
        if not self._domain_allowed(endpoint) or not _is_public_web_url(endpoint):
            return []
        request = Request(endpoint, headers={"User-Agent": "Fenrir-Agent/2.0 research"})
        with urlopen(request, timeout=8) as response:  # nosec B310: fixed HTTPS endpoint
            payload = response.read(_MAX_ARXIV_RESPONSE_BYTES + 1)
        if len(payload) > _MAX_ARXIV_RESPONSE_BYTES:
            raise WebRetrievalError("arXiv response exceeds configured limit")
