...
from fenrir_agent.logger import configure_logging
...
FENRIR_CHAT_PROMPT = (
    "You are inside Fenrir Agent, a helpful AI assistant agentic CLI designed to help people learn, build, understand, create, and solve problems. You can speak many languages fluently."
)
...
SYSTEM_PROMPTS = {
    "auto": FENRIR_CHAT_PROMPT,
}

DEFAULT_PROMPT = FENRIR_CHAT_PROMPT
...
class FileHandler:
    ...
    def load_image(self, path: Path) -> Tuple[bool, Optional[Any], str]:
        ...
        try:
            ...
            from fenrir_agent.media import load_model_image
    ...
    def get_clipboard_image(self) -> Optional[ImageAttachment]:
        ...
        if Image is not None and isinstance(clipped, Image.Image):
            from fenrir_agent.media import normalize_image
...
class FenrirAgentEngine:
    ...
    def _llama_cpp_url() -> str:
        url = os.environ.get(
            "FENRIR_LLAMA_CPP_URL", "http://127.0.0.1:8080/v1"
        ).rstrip("/")
        if urlparse(url).hostname not in {"127.0.0.1", "localhost", "::1"}:
            raise ValueError("FENRIR_LLAMA_CPP_URL must use a local loopback host")
        return url
    ...
    def _llama_cpp_startup_timeout() -> float:
        ...
        raw_value = os.environ.get(
            "FENRIR_LLAMA_CPP_STARTUP_TIMEOUT", "900"
        )
        try:
    ...
    def _llama_cpp_download_failure_limit() -> int:
        ...
        raw_value = os.environ.get(
            "FENRIR_LLAMA_CPP_DOWNLOAD_FAILURE_LIMIT", "1"
        )
        try:
    ...
    def _start_llama_cpp_server(
        ...
        try:
            self._llama_log_path = Path(tempfile.gettempdir()) / "fenrir-llama-server.log"
            ...
            # generation interrupt in FenrirAgent; it must not kill the loaded model.
            creationflags = 0
            if os.name == "nt":
                creationflags = (
            ...
            if model_ids and not any(
                ...
                if self._llama_cpp_is_ready(base_url):
                    return False, (
                        ...
                        "FenrirAgent could not stop that server."
                    )
                server_ready = False
        if not server_ready:
    ...
    def _stop_unmanaged_windows_llama_cpp_server(self) -> None:
        """Stop only a local llama executable listening on FenrirAgent's configured port."""
        if os.name != "nt":
            return
        try:
        ...
        try:
            ...
            for token in streamer:
                ...
                if display_token:
                    ...
                    if "You are Fenrir Agent" in display_token or "by Biwa" in display_token:
...
def get_engine() -> FenrirAgentEngine:
    global _engine
    if _engine is None:
        _engine = FenrirAgentEngine()
    return _engine
