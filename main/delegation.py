...
class DelegationManager:
    ...
    KEEP_JOBS = 100
    EXCLUDED_PARTS = {
        ".git", ".opencli", ".venv", "node_modules", "__pycache__"
    }
    ...
    def __init__(
        ...
        *,
        ...
        self.root = (root or Path.home() / ".opencli" / "delegates") / digest
        self.state_path = self.root / "jobs.json"
        self.snapshots = self.root / "snapshots"
        self.executor = executor
        self._lock = threading.RLock()
        self._pool = ThreadPoolExecutor(max_workers=1, thread_name_prefix="opencli-delegate")
    ...
    def _recover_interrupted(self) -> None:
        ...
        for job in self._jobs.values():
            if job.status in {"queued", "running", "cancelling"}:
                job.status = "failed"
                job.error = "OpenCLI exited before delegate completed."
