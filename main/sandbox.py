...
class SandboxConfig:
    ...
    excluded_patterns: tuple[str, ...] = (
        ".git", ".git/**", ".opencli", ".opencli/**", ".env", ".env.*",
...
class DockerSandbox:
    ...
    def run(
        ...
        """Execute argv without a host shell or network access."""
        ...
        try:
            ...
            return {"error": "Sandbox command timed out.", "backend": self.backend}
...
class E2BSandbox:
    ...
    E2B remains optional. OpenCLI never stores API keys. Workspace transfer is
    ...
    def _sdk() -> Any:
        try:
            ...
            raise RuntimeError(
                "E2B SDK unavailable. Install with: pip install 'opencli[sandbox]'"
            ) from error
        return Sandbox
    ...
    def _excluded(self, relative: str) -> bool:
        ...
        if any(part in {".git", ".opencli", ".venv", "node_modules", "__pycache__"} for part in parts):
            return True
            ...
            for relative in changed:
                ...
                try:
                    with tempfile.NamedTemporaryFile(
                        mode="wb",
                        prefix=f".{target.name}.opencli-e2b-",
                        suffix=".tmp",
                        dir=target.parent,
                        delete=False,
                    ) as output:
